...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		tb, err := gotable.Create("topic", "partition",
			"leader broker id", "leader broker address", "err")
		if err != nil {
		...
		for _, partition := range partitions {
			...
			if err != nil {
				...
				row["leader broker id"] = strconv.Itoa(int(broker.ID()))
				row["leader broker address"] = broker.Addr()
			}
